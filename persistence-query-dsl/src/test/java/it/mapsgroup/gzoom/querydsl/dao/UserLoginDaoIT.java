...
public class UserLoginDaoIT extends AbstractDaoTest {
    @Autowired
    ...
    public void name() throws Exception {
        ...
        List<Tuple> s = queryFactory.select(
                userLogin.userLoginId, party.partyName)
                .from(userLogin)
                .join(userLogin.userParty, party)
                .join(party._personParty, person)
                .join(userLogin.userParty, party2)
                /*.where(
                        userLogin.enabled.isTrue().or(
                                Expressions.allOf(
                                        Expressions.allOf(party.isUnread, party.partyId.isNotEmpty())
                                        , Expressions.anyOf(party.partyId.between("ss", "rr"), userLogin.isSystem)
                                )
                        )
                                .and(userLogin.userLoginId.likeIgnoreCase("%ADMIN%"))
                                .and(party.partyName.like("aaa"))
                ).fetch();
*/.fetch();
        s.size();
    }
