...
public class QWorkEffortRevision extends com.querydsl.sql.RelationalPathBase<WorkEffortRevision> {
    ...
    public final com.querydsl.sql.PrimaryKey<WorkEffortRevision> workEffortRevisionPk = createPrimaryKey(workEffortRevisionId);
    ...
    public QWorkEffortRevision(String variable) {
        super(WorkEffortRevision.class, forVariable(variable), "DBO", "WORK_EFFORT_REVISION");
        addMetadata();
    }
    ...
    public QWorkEffortRevision(Path<? extends WorkEffortRevision> path) {
        super(path.getType(), path.getMetadata(), "DBO", "WORK_EFFORT_REVISION");
        addMetadata();
    }

    public QWorkEffortRevision(PathMetadata metadata) {
        super(WorkEffortRevision.class, metadata, "DBO", "WORK_EFFORT_REVISION");
        addMetadata();
    }
    ...
    public void addMetadata() {
        ...
        addMetadata(createdStamp, ColumnMetadata.named("CREATED_STAMP").withIndex(17).ofType(Types.TIMESTAMP).withSize(23).withDigits(3));
        addMetadata(createdTxStamp, ColumnMetadata.named("CREATED_TX_STAMP").withIndex(18).ofType(Types.TIMESTAMP).withSize(23).withDigits(3));
        addMetadata(description, ColumnMetadata.named("DESCRIPTION").withIndex(5).ofType(Types.VARCHAR).withSize(255).notNull());
        addMetadata(fromDate, ColumnMetadata.named("FROM_DATE").withIndex(10).ofType(Types.TIMESTAMP).withSize(23).withDigits(3));
        ...
        addMetadata(lastUpdatedStamp, ColumnMetadata.named("LAST_UPDATED_STAMP").withIndex(15).ofType(Types.TIMESTAMP).withSize(23).withDigits(3));
        addMetadata(lastUpdatedTxStamp, ColumnMetadata.named("LAST_UPDATED_TX_STAMP").withIndex(16).ofType(Types.TIMESTAMP).withSize(23).withDigits(3));
        addMetadata(organizationId, ColumnMetadata.named("ORGANIZATION_ID").withIndex(12).ofType(Types.VARCHAR).withSize(20));
        addMetadata(refDate, ColumnMetadata.named("REF_DATE").withIndex(4).ofType(Types.TIMESTAMP).withSize(23).withDigits(3).notNull());
        addMetadata(toDate, ColumnMetadata.named("TO_DATE").withIndex(11).ofType(Types.TIMESTAMP).withSize(23).withDigits(3));
