 ...
 * @author Andrea Fossi
 */
public class FilterMatcher2 {
    private static final String module = FilterMatcher2.class.getName();
    ...
    public FilterMatcher2(Map<String, Object> authorization, Map<String, Object> filter, FilterMatcherHelper filterMatcherHelper, GeoHelper geoHelper) {
    ...
    private boolean matchTerm(Map<String, Object> term) throws GenericServiceException, GenericEntityException {
     ...
     */
    @SuppressWarnings("unchecked")
    private boolean matchOperation(Map<String, Object> itemTerm) throws GenericServiceException, GenericEntityException {
        if (UtilValidate.isNotEmpty(itemTerm.get("operationIds")) && UtilValidate.isEmpty(itemTerm.get("operations"))) {
            List<Map<String, Object>> operations = FastList.newInstance();
            for (String id : (List<String>) itemTerm.get("operationIds"))
                operations.add(filterMatcherHelper.findOperation(id));
            itemTerm.put("operations", operations);
        }
     ...
     */
    @SuppressWarnings("unchecked")
    private boolean matchPackagings(Map<String, Object> itemTerm) throws GenericServiceException, GenericEntityException {
        if (UtilValidate.isNotEmpty(itemTerm.get("packagingIds")) && UtilValidate.isEmpty(itemTerm.get("packagings"))) {
            List<Map<String, Object>> operations = FastList.newInstance();
            for (String id : (List<String>) itemTerm.get("packagingIds"))
                operations.add(filterMatcherHelper.findPackaging(id));
            itemTerm.put("packagings", operations);
        }
    ...
    private boolean matchRegistrationNumbers(Map<String, Object> itemTerm) throws GenericServiceException {
        if (UtilValidate.isNotEmpty(itemTerm.get("registrationNumberIds")) && UtilValidate.isEmpty(itemTerm.get("registrationNumbers"))) {
            List<Map<String, Object>> operations = FastList.newInstance();
            for (String id : (List<String>) itemTerm.get("registrationNumberIds"))
                operations.add(UtilMisc.toMap("registrationNumber", (Object) id));
            itemTerm.put("registrationNumbers", operations);
        }
     ...
     */
    @SuppressWarnings("unchecked")
    private boolean matchWastes(Map<String, Object> itemTerm) throws GenericServiceException, GenericEntityException {
        Map<String, Object> filterTerm = getTerm(AgreementTermTypeOfbiz.GN_CNS_WASTE.name());
        if (filterTerm == null) {
            append("**[true] wasteTerm  not found");
            return true;
        }
        if (UtilValidate.isNotEmpty(itemTerm.get("wasteCerCodeIds")) && UtilValidate.isEmpty(itemTerm.get("wasteCerCodes"))) {
            List<Map<String, Object>> wasteCerCodes = FastList.newInstance();
            for (String id : (List<String>) itemTerm.get("wasteCerCodeIds"))
                wasteCerCodes.add(filterMatcherHelper.findWasteCerCode(id));
            itemTerm.put("wasteCerCodes", wasteCerCodes);
        }
    ...
    private Map<String, Object> getTerm(String termTypeId) {
        ...
        for (Map<String, Object> term : terms)
            if (areEqual(term.get("termTypeId"), termTypeId)) return term;
        return null;
    }
    ...
    private boolean contains(Object valueToMatch, Collection<Map<String, Object>> entries, String key) {
        for (Map<String, Object> entry : entries) {
            ...
            if (areEqual(valueToMatch, value)) return true;
        }
        return false;
    }
